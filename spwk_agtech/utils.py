...
COL = [
    'IRRAD', 'TMIN', 'TMAX', 'VAP', 'RAIN', 'E0', 'ES0', 'ET0', 'WIND',
    'IRRIGATE', 'N', 'P', 'K'
]
...
def NASAPowerWeatherDataFetcher(latitude,
                                longitude,
                                force_update=False,
                                ETmodel='PM',
                                fill='ffill'):
    ...
    weather = NASAPowerWeatherDataProvider(latitude, longitude, force_update,
                                           ETmodel)

    # Fill the missing data
    if weather.missing > 0:
        logging.debug(f'there are(is) {weather.missing} missing value(s) in weather data')
        df_weather = pd.DataFrame(weather.export())
        full_range = pd.date_range(start=df_weather.DAY.min(),
                                   end=df_weather.DAY.max())
        full_range_weather = df_weather.set_index('DAY').reindex(
            full_range).rename_axis('DAY').reset_index()
        if fill == ('ffill' or 'bfill'):
            filled_weather = full_range_weather.fillna(method=fill, axis=0)
        else:
            filled_weather = full_range_weather.interpolate(method='linear',
                                                            axis=0)
...
def send_actions2engine(actions, engine):
    ...
    if np.isfinite(actions[9]):
        ...
        logging.warning(
            f'Irrigation action is {actions[9]}. You should check actor.')
        irrigate_act = {'amount': actions[9], 'efficiency': 0.7}
...
def plot_pcse_env_obs(env, policies: list, policy_name: list, test: list):
    ...
    print('{0:15s}: return ({1:>9.3f}) / net profit ({2:>10.3f}) '.format(
...
def plot_pcse_env_act(env, policies: list, policy_name: list, test: list):
    ...
    act_df = pd.DataFrame(denorm_act,
                          index=pd.date_range(datetime.date(1988, 1, 2),
                                              periods=196).tolist(),
                          columns=COL)
...
def plot_pcse_engine(output: dict, output_varname: dict = {None}):
    ...
    for ix, var in enumerate((output_varname.values())):
        try:
            ...
            plt.setp(ax[ix].yaxis.get_label(),
                     rotation=0,
                     fontsize=10,
                     position=(0, 1.02))
